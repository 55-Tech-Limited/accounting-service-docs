    ...
    def create_account():
        try:
            response = requests.post(
                f"{baseUrl}/api/auth/register/account",
...
<Tabs groupId="programming-language">
  ...
  <TabItem value="python" label="Python">
    ...
    def login_account():
        try:
            response = requests.post(
                f"{baseUrl}/api/auth/login/account",
                headers={'Content-Type': 'application/json'},
                json={
                    'email': 'owner@mybetting.com',
                    'password': 'secure_password_here'
...
<Tabs groupId="programming-language">
  ...
  <TabItem value="python" label="Python">
    ...
    def generate_api_keys(token):
        try:
            response = requests.post(
                f"{baseUrl}/api/auth/account/generate-keys",
                headers={
...
To fund a user account, you need to send a POST request to the `/api/transactions/fund-user/` endpoint with the amount and either the `user_id` or `user_reference`.
...
auth_header = f"Basic {auth['apiKeys']['base64_encoded']}"
        ...
        response = requests.post(
            f"{baseUrl}/api/transactions/fund-user/",
            headers={
                'Authorization': auth_header,
                'Content-Type': 'application/json'
            },
...
- `/api/account/user/:reference/reference`
...
def check_user_balance(auth, identifier, is_reference=False):
    try:
        ...
        url = f"{base_url}/api/account/user/{identifier}/reference" if is_reference \
            else f"{base_url}/api/account/user/{identifier}"
...
To place a bet offer, you need to send a POST request to the `/api/bets/make-offer` endpoint with the bet details. You can either use the `user_id` or `user_reference` to make a bet offer.
...
def place_bet_offer(auth):
    try:
        ...
        import requests
        response = requests.post(
            f"{base_url}/api/bets/make-offer",
            headers={
                'Authorization': auth_header,
                'Content-Type': 'application/json'
            },
...
To view open bet offers, you need to send a GET request to the `/api/bets/open-bets` endpoint.
...
To accept a bet offer, you need to send a POST request to the `/api/bets/accept-offer` endpoint with the bet offer details. The `requesting_user_id`, `requesting_user_reference`, and `meta` are optional. If they are not provided, the system will determine the bets to be accepted. To define the accepting user, use either the `accepting_user_id` or the `accepting_user_reference`.
...
def accept_bet_offer(auth):
    try:
        ...
        import requests
        response = requests.post(
            f"{base_url}/api/bets/accept-offer",
            headers={
                'Authorization': auth_header,
                'Content-Type': 'application/json'
            },
...
To update a wager outcome, you need to send a POST request to the `/api/bets/update-wager-outcome` endpoint with the wager reference and the outcome. The
...
def update_wager_outcome(auth):
    try:
        ...
        import requests
        response = requests.post(
            f"{base_url}/api/bets/update-wager-outcome",
            headers={
                'Authorization': auth_header,
                'Content-Type': 'application/json'
            },
...
To get bet history, you need to send a GET request to the `/api/bets/history` endpoint.
